...
// Factory method to create an instance of FluxMeter
static G4VSensitiveDetector* create_flux_sd(const std::string& aDetectorName,
                                            dd4hep::Detector& aLcdd) {
  std::string readoutName = aLcdd.sensitiveDetector(aDetectorName).readout().name();
  return new det::FluxMeter(
      aDetectorName, readoutName, aLcdd.sensitiveDetector(aDetectorName).readout().segmentation());
}
}
}
...
DECLARE_EXTERNAL_GEANT4SENSITIVEDETECTOR(FluxMeter, dd4hep::sim::create_flux_sd)
